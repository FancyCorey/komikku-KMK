...
1. **Shizuku prompts intrude when not needed** â€” Shizuku setup card and status messages appeared for users on PRIVATE or CURRENT installer who never selected Shizuku mode.
2. **Quarantined/blocked cards dominate the screen** â€” `UnsafeSourcesCard` and `BlockedPackagesCard` sat at the top of the page, overshadowing normal evaluation controls.
3. **No persistence across screen navigation** â€” leaving Source Evaluation cancelled the evaluation run.
4. **No taste-profile confidence signal** â€” evaluation ran with the same confidence regardless of how sparse the taste profile was.

## Solution

Four phases implemented in one pass.

### Phase 0 â€” Taste Profile Confidence
...
### Phase 2 â€” Safety Diagnostics Demotion
...
Dialogs, `UnsafeSourcesCard`, `BlockedPackagesCard`, `UnsafeSourceRow`, `BlockedPackageRow` composables are unchanged â€” only their entry points changed from inline cards to dialog triggers.

### Phase 3 â€” Background Execution
...
- `fun reset()` â€” clears all fields and state
...
- Collects `runner.state` via `.onEach { ... }.first { it.isTerminal }` â€” updates singleton state and notification per emission
...
- `ExistingWorkPolicy.KEEP` â€” prevents double-enqueue if screen is opened twice
- `onStopped()` NOT overridden (final in `CoroutineWorker`; cancellation handled via CancellationException)

**`SourceEvaluationNotifier`**:
- `buildProgressNotification()` â€” ongoing notification with progress bar and current extension name
- `updateProgress()` â€” posts to `ID_SOURCE_EVALUATION_PROGRESS`
- `dismissProgress()` â€” cancels progress notification
- `showComplete(strongFitCount)` â€” auto-cancel complete notification with strong-fit count
- Uses `context.stringResource(KMR.strings.xxx)` (moko-resources) â€” not `context.getString(R.string.xxx)`
...
- **No DB schema changes for Phase 3** â€” process-scoped singleton is safe because WorkManager runs in-process. On process restart, the job fails with a descriptive error: "Evaluation state was lost (app process restarted). Please start again." No migration needed.
- **No notification cancel action** â€” cancellation is available from the Source Evaluation screen via the Cancel button.
- **Shizuku OR-logic** â€” `showShizukuSetup = s.showShizukuSetup || autoShowShizuku` ensures a user who manually expands the card is not collapsed by mode refresh.
- **v0.6.18 safety behavior intact** â€” `KnownUnsafeExtensionPackages` static guard, `ExtensionLoader` filter, and `BlockedPackagesCard` dialogs all remain. Only the placement changed (dialogs instead of inline cards).
...
- `source_evaluation_low_confidence_warning` â€” low-confidence taste profile banner
- `source_evaluation_shizuku_setup_toggle` â€” compact toggle link text
- `source_evaluation_safety_diagnostics_title` â€” SafetyDiagnosticsRow section header
- `source_evaluation_safety_quarantined_count` â€” "Quarantined: %1$d"
- `source_evaluation_safety_blocked_count` â€” "Blocked: %1$d"
- `source_evaluation_safety_show_quarantined` â€” TextButton label
- `source_evaluation_safety_show_blocked` â€” TextButton label
- `source_evaluation_job_notification_title` â€” "Source evaluation running"
...
- `TasteProfileConfidenceTest` â€” 11 tests, all PASSED
