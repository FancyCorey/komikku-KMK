# KMK-Recs v0.7.17 — Bundle Ambiguity, Shizuku UX, String Audit
...
## Task 1 — R-010: Bundle Import Source Ambiguity
...
- `Unambiguous(ext)` — exactly one match
- `Ambiguous(candidates)` — two or more matches
- `NotFound` — no pkgName in item, or no available extension matches
...
- `ImportItemStateBadge` handles `AmbiguousSource` → "Ambiguous source" label.
- `InstallExtensionCard` accepts an optional `labelRes: StringResource` (defaults to `rec_bundle_missing_source_prompt`).

### Files Changed

- `app/src/main/java/exh/recs/share/RecommendationBundleSourceResolver.kt` — added `AvailableExtensionResolution`, `resolveAvailableExtension()`
- `app/src/main/java/exh/recs/share/RecommendationBundleImportScreenModel.kt` — added `AmbiguousSource` state, replaced `findAvailableExtension()` with resolver call
- `app/src/main/java/exh/recs/share/RecommendationBundleImportScreen.kt` — added `AmbiguousSource` badge + install cards, `labelRes` param on `InstallExtensionCard`
- `i18n-kmk/.../base/strings.xml` — added `rec_bundle_import_state_ambiguous_source`, `rec_bundle_ambiguous_source_prompt`
- `app/src/test/java/exh/recs/share/RecommendationBundleSourceResolverTest.kt` — 6 new tests
...
## Task 2 — R-005: Shizuku UX Typed Messages
...
- `app/src/main/java/exh/recs/evaluation/SourceEvaluationInstallerPolicy.kt` — added `InstallerPolicyMessage` enum, changed `message: String?` → `messageKey: InstallerPolicyMessage?`
- `app/src/main/java/exh/recs/evaluation/SourceEvaluationScreen.kt` — added `toLocalString()` composable extension, updated 3 usages
- `i18n-kmk/.../base/strings.xml` — 8 new installer policy strings

---

## Task 3 — R-019: Bundle Load Error String Audit
...
Changed `State.LoadError(val message: String)` → `State.LoadError(val error: LoadErrorKey)`.
...
- `app/src/main/java/exh/recs/share/RecommendationBundleImportScreenModel.kt` — added `LoadErrorKey`, changed `State.LoadError`
- `app/src/main/java/exh/recs/share/RecommendationBundleImportScreen.kt` — added `LoadErrorKey.toLocalString()`, updated Text
- `i18n-kmk/.../base/strings.xml` — 7 new load error strings
...
- Phase 12 public packaging — deferred.
