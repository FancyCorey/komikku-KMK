...
- **Local Source exclusion still intentional**: `RecommendationSourceFilter` excludes `id == 0L` via `includeLocal = false`. This is correct â€” Local Source is for local manga files, not a recommendation engine. Phase 3 (real Local Source) was not implemented.
...
### Phase 2 â€” Combined Picks Row

**`CombinedPicksAccumulator.kt`** (new file)

- Pure class, not thread-safe internally â€” callers synchronize with `accumulatorLock`.
- Keyed by `"${manga.source}:${manga.url}"` â€” same-title manga from different source+url are never merged.
...
- Row header click is no-op (first pass â€” drill-down not implemented).
- Individual manga tap works normally.
- Combined row is only shown when `combinedResult` is a non-empty `Success`.
- Combined row is NOT passed through `dedupedItems()` â€” each row dedupes independently.
...
- `CombinedPicksAccumulatorTest.kt` â€” 12 new tests, all pass
...
- Combined Picks is rebuilt from scratch on each `load()` â€” it does not cache independently. If per-source rows load from cache, Combined Picks is rebuilt from those cached results. This is correct behavior per plan.
