...
- Private extensions are NOT registered with Android's package manager â†’ `isPackageInstalled()` returns `false` â†’ silent private file deletion via `ExtensionLoader.uninstallPrivateExtension()`.
- System-installed extensions ARE registered â†’ `isPackageInstalled()` returns `true` â†’ Android `ACTION_UNINSTALL_PACKAGE` intent (shows dialog).
...
Prior to v0.6.13, `cleanupExtension()` called `extensionManager.uninstallExtension(installedExt)` without checking `installedExt.isShared`. If the evaluation used Shizuku or Current (non-private) installer, the extension was system-installed â†’ `isShared=true` â†’ `isPackageInstalled()=true` â†’ Android uninstall dialog.
...
3. `cleanupDecision(false, true, false)` â†’ `RemovePrivateSilently`.
4. `uninstallApk()` sees `!isPackageInstalled()` â†’ deletes private file silently. âœ“
...
Decision tree:
1. `!installedAfterEvaluation` â†’ `NotNeeded` (install failed or already cleaned up)
2. `preExistingInstalled` â†’ `SkipPreExisting` (protect user-installed extensions)
3. `!isShared` â†’ `RemovePrivateSilently` (private file, silent deletion)
4. `isShared` â†’ `PromptRequired` (system-installed, would trigger Android dialog)
...
- `PromptRequired` + `!promptHeavyCleanupAllowed` â†’ skip uninstall, log the skip, record `CleanupStatus.PromptRequired`.
- `PromptRequired` + `promptHeavyCleanupAllowed` â†’ call `extensionManager.uninstallExtension()` (Android dialog), record `CleanupStatus.PromptRequired`.
...
- Added `AlertDialog` for the prompt-heavy warning: shows title, message, and three actions â€” "Use Private" (if privateAvailable), "Cancel" (dismiss), "Continue anyway" (confirmAndStartWithPrompts).
...
- `app/src/main/java/exh/recs/evaluation/SourceEvaluationQueueState.kt` â€” CleanupStatus, EvaluationResult.cleanupStatus, promptHeavyCleanupAllowed
- `app/src/main/java/exh/recs/evaluation/SourceEvaluationRunner.kt` â€” pre-existing detection, diagnostics, private-aware cleanup
- `app/src/main/java/exh/recs/evaluation/SourceEvaluationInstallerPolicy.kt` â€” SHIZUKU requiresPromptWarning fix, PRIVATE description
- `app/src/main/java/exh/recs/evaluation/SourceEvaluationScreenModel.kt` â€” dialog state, launchEvaluation, switchToPrivate
- `app/src/main/java/exh/recs/evaluation/SourceEvaluationScreen.kt` â€” AlertDialog, chip label, ShizukuSetupCard updates
- `app/src/main/java/exh/recs/KmkRecsReleaseNotes.kt` â€” VERSION_CODE=613
...
- `docs/recommendations/README.md` â€” added v0.6.13 implementation report entry
- `RECOMMENDATION_VERSIONING.md` â€” added v0.6.13 entry

## Tests

- `exh.recs.evaluation.SourceEvaluationCleanupPolicyTest` â€” 6 tests, all PASSED
...
- **Test A**: Private evaluation of fresh non-installed extension â†’ confirm no install or uninstall prompt; confirm logcat shows `isShared=false` and `decision=RemovePrivateSilently`.
- **Test B**: Shizuku evaluation â†’ confirm warning dialog appears before start; confirm logcat shows `isShared=true` and `decision=PromptRequired`; confirm extension is NOT uninstalled if "Continue anyway" was not chosen.
- **Test C**: Pre-existing protection â†’ install extension normally; ensure Source Evaluation skips it and does not uninstall it.
- **Test D**: Private failure â†’ confirm no automatic Shizuku fallback.

## Verification Conclusion

Based on code analysis:
- Private installer IS verified to work architecturally for fresh temporary extension evaluation â€” it installs without a prompt and removes silently.
- The prior uninstall prompts were caused by SHIZUKU/CURRENT mode (not PRIVATE) â€” those modes system-install extensions, causing `isShared=true` and an Android dialog during cleanup.
