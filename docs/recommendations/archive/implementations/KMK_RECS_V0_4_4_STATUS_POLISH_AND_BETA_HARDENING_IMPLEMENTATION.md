# KMK-Recs v0.4.4 Status Polish and Beta Hardening â€” Implementation Report
...
### Phase 1 â€” HiddenByDuplicateHandling status
...
- `statuses: Map<Long, RecommendationSourceRunStatus>` â€” current per-source statuses after the For You batch loop
- `sourceHasVisibleResults: Map<Long, Boolean>` â€” whether each source has at least one card visible after cross-source display dedupe
...
### Phase 2 â€” Live settings freshness
...
### Phase 4 â€” Top Picks polish

`TopPicksScreen` (added in v0.4.3) updated in v0.4.4:

- Added `isPartial: Boolean = false` constructor parameter.
- `TopPicksScreenModel.State` adds `isLoading: Boolean = true` â€” starts true until manga IDs are loaded from the DB.
...
- `app/src/main/java/exh/recs/RecommendationSourceRunStatus.kt` â€” added `HiddenByDuplicateHandling` enum value
- `app/src/main/java/exh/recs/RecommendationStatusAdjuster.kt` (new) â€” pure `adjustStatusesForDedupe()` helper
- `app/src/main/java/exh/recs/BrowsePersonalRecommendationsScreenModel.kt` â€” post-dedupe adjustment after batch loop
- `app/src/main/java/exh/recs/settings/RecommendationsSettingsScreenModel.kt` â€” `changes()` live subscription in init
- `app/src/main/java/exh/recs/settings/RecommendationsSettingsScreen.kt` â€” `rec_source_status_note` item + `HiddenByDuplicateHandling` case
- `app/src/main/java/exh/recs/TopPicksScreen.kt` â€” `isPartial` param, loading state, empty state, partial banner
- `app/src/main/java/exh/recs/BrowsePersonalRecommendationsTab.kt` â€” `isPartial` passed to `TopPicksScreen`
- `app/src/main/java/exh/recs/KmkRecsReleaseNotes.kt` â€” VERSION_CODE=404
- `i18n-kmk/src/commonMain/moko-resources/base/strings.xml` â€” 4 new strings: `rec_source_status_duplicate_hidden`, `rec_source_status_note`, `rec_top_picks_partial`, `rec_top_picks_empty`
- `app/src/test/java/exh/recs/RecommendationSourceRunStatusStoreTest.kt` â€” added `HiddenByDuplicateHandling` round-trip test (10 tests total)
- `app/src/test/java/exh/recs/AdjustStatusesForDedupeTest.kt` (new) â€” 9 tests

## Tests Run

- `exh.recs.RecommendationSourceRunStatusStoreTest` â€” 10 tests, all PASSED
- `exh.recs.AdjustStatusesForDedupeTest` â€” 9 tests, all PASSED
