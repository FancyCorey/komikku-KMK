...
Date: 2026-06-28 (updated: 2026-06-28 — all product decisions resolved; 5 deferred items promoted to phases; AniList cache removed)

Status: planning document. All 7 master plan phases complete as of v0.7.20. This document captures everything remaining. Implement one phase at a time; do not start a phase without explicit user approval.

---

## Overview

| Phase | Version | Topic | Status |
|---|---|---|---|
| A | v0.7.21 | Quick UX polish (timestamps, pull-to-refresh, live updates, quarantine toggle) | Pending |
| B | v0.7.22 | Cross-source link group management UI | Pending |
| C | v0.7.23 | Source Evaluation UX polish (error categories, retry button, rec-quality triggers) | Pending |
| D | v0.7.24 | Fit stat improvements (decay, Top Picks contribution) | Pending |
| E | v0.7.25 | For You offline / connection loss handling | **Complete** |
| F | v0.7.26 | Minimum chapter count filter | **Complete** |
| G | v0.7.27 | Quality signal display UI (Best Version history browser) | Pending |
| H | v0.7.28 | Backup / restore for seen manga | Pending |
| I | v0.7.29 | Fullscreen preview improvements (rotation, tap-to-close, fit toggle) | Pending |
| J | v0.7.30 | Enrichment cap: make configurable | Pending |
...
**Files:**
- `app/src/main/java/exh/recs/settings/RecommendationsSettingsScreen.kt` — update `SourcePriorityItem` status line to include relative age when `fitStats?.updatedAt > 0`
- `i18n-kmk/.../base/strings.xml` — 2–3 strings (`rec_source_last_checked_today`, `rec_source_last_checked_days_ago`, `rec_source_never_checked`)
...
**Files:**
- `app/src/main/java/exh/recs/BrowsePersonalRecommendationsTab.kt` — add pull-to-refresh wrapper around the source list; bind to `state.isLoading` and `screenModel::refresh`
...
**How:** Replace the one-shot `init` load in `LovedMangaScreenModel` with a `Flow` subscription — subscribe to `getMangaTaste.subscribeAll()` or an equivalent reactive source and re-apply the grouper on each emission.

**Files:**
- `app/src/main/java/exh/recs/LovedMangaScreenModel.kt` — replace one-shot await with a `collectLatest` flow subscription

**Constraints:** The grouper (`LovedMangaGrouper`) is pure and idempotent — safe to re-run on each emission. Sorting preference changes already re-filter in state; the live-update only needs to react to underlying data changes, not preference changes.
...
**Files:**
- `app/src/main/java/exh/recs/evaluation/SourceEvaluationScreen.kt` — modify `SafetyDiagnosticsRow` or its quarantine sub-section to support collapse
...
- Deleting a link does NOT change any ratings — links and ratings are independent.
...
### C1 — Per-source error category labels in past results

**What:** Source Evaluation past result rows show raw error messages. `SourceRecommendationFitFailureClassifier` (v0.7.13) already categorizes errors into a 14-value enum, but past result rows don't display the category — only the raw message.
...
**Files:**
- `app/src/main/java/exh/recs/evaluation/SourceEvaluationScreen.kt` — update past-result error display to use `SourceRecommendationFitFailureClassifier.classify()`
- `i18n-kmk/.../base/strings.xml` — localized category label strings for each `FailureCategory` value (14 values, some can share strings)
...
**Files:**
- `app/src/main/java/exh/recs/evaluation/SourceEvaluationScreen.kt` — add conditional "Retry (N remaining)" button when status is `ConnectivityLost` and remaining candidates exist
- `app/src/main/java/exh/recs/evaluation/SourceEvaluationScreenModel.kt` — expose a `retryAfterConnectivityLoss()` action that delegates to the existing continue-batch path
...
### C3 — Auto-prompt rec-quality re-check after taste profile change

**What:** After the user changes tag preferences or ratings, the rec-quality verdicts for Source Evaluation past results may be stale. Currently the "Re-check all" button requires manual action. Add a non-blocking prompt: "Your taste profile has changed — re-check recommendation quality?"
...
**Files:**
- `app/src/main/java/exh/recs/evaluation/SourceEvaluationScreenModel.kt` — compute and cache a profile fingerprint at rec-quality evaluation time; compare on screen open
- `app/src/main/java/exh/recs/evaluation/SourceEvaluationScreen.kt` — show conditional info card when fingerprint mismatch detected
- `app/src/main/java/eu/kanade/domain/source/service/SourcePreferences.kt` — add `sourceEvaluationLastRecQualityProfileFingerprint()` preference
...
### D1 — Fit stat time decay

**What:** `SourceFitStats` is currently pure accumulation — an error run from 3 months ago carries the same weight as one from yesterday. This makes labels slow to recover after a source improves (e.g., after an extension update fixes errors).
...
**Files:**
- `app/src/main/java/exh/recs/SourceFitStats.kt` — add recent-window fields, update `merge()`, update `fitLabel` to prefer recent rates

**Constraints:** Serialization must remain backward-compatible — new fields default to 0 on parse from old data (safe with the existing `parts.size < N` guards).
...
**Files:**
- `app/src/main/java/exh/recs/BrowsePersonalRecommendationsScreenModel.kt` — collect `sourceIds` from ranked Top Picks result and pass to `mergeRun()`
- `app/src/main/java/exh/recs/SourceFitStats.kt` — add `topPicksContributionCount` field; update `merge()` and `fitLabel` to weight it positively
...
- The offline state shows a message ("No connection — For You needs internet to load recommendations") and a Retry button that re-checks connectivity and restarts the run if online.
- If connectivity returns while the screen is open, optionally show a snackbar: "Back online — tap to refresh."
...
**Files:**
- `app/src/main/java/exh/recs/BrowsePersonalRecommendationsScreenModel.kt` — add offline pre-check before starting a run; add `isOffline: Boolean` (or typed error) to State
- `app/src/main/java/exh/recs/BrowsePersonalRecommendationsTab.kt` — render offline state composable
- `i18n-kmk/.../base/strings.xml` — `rec_offline_title`, `rec_offline_body`, `rec_offline_retry`
...
**What:** For You currently shows manga with any chapter count, including one-shots or series with 0 locally-known chapters. Add a user-configurable minimum chapter count filter that uses only already-locally-known chapter counts — no network fetch.
...
- "Locally known" means the chapter count stored in the local DB. If a manga has never been opened (count = 0 or null), treat 0 as unknown — do not filter out unless threshold = 0 (i.e., explicit "any count including 0" selection).

**Files:**
- `app/src/main/java/eu/kanade/domain/source/service/SourcePreferences.kt` — add `recommendationMinChapterCount()` preference (Int, default 0)
- `app/src/main/java/exh/recs/BrowsePersonalRecommendationsScreenModel.kt` — read preference and pass to scorer/filter step
- `app/src/main/java/exh/recs/PersonalRecommendationScorer.kt` (or equivalent post-fetch filter) — add chapter count exclusion check
- `app/src/main/java/exh/recs/settings/RecommendationsSettingsScreen.kt` — add minimum chapter count setting row
- `i18n-kmk/.../base/strings.xml` — `rec_min_chapter_count`, `rec_min_chapter_count_off`, etc.

**Constraints:**
- Never fetch chapter data. Use only `manga.totalChapters` or equivalent local DB field.
- A manga with null/unknown chapter count is NOT filtered out — only those with a known count below the threshold are excluded.
...
**Required behavior:**
- Per-manga view: shows each recorded quality signal row — source name, verdict (Best / Not Best / Unknown), recorded date.
...
- `i18n-kmk/.../base/strings.xml` — strings for the screen title, verdict labels, delete confirmation
...
- Existing `GetMangaSourceQualitySignals` interactor is already present — use it.
...
**Files:**
- `app/src/main/proto/` — add `BackupSeenMangaKey` message; add repeated field to the root backup message
- `app/src/main/java/eu/kanade/tachiyomi/data/backup/create/creators/TasteBackupCreator.kt` — serialize seen keys
- `app/src/main/java/eu/kanade/tachiyomi/data/backup/restore/restorers/TasteBackupRestorer.kt` — merge restored keys
- `app/src/test/java/exh/recs/backup/TasteBackupRoundTripTest.kt` — new round-trip test for seen manga keys
...
**How:** Change `var fullscreenPage by remember { mutableStateOf<Int?>(null) }` to `rememberSaveable`. No other changes needed — the page index is a primitive (Int) that survives the bundle.

**Files:**
- `app/src/main/java/exh/recs/CrossExtensionMatchScreen.kt` (or wherever the fullscreen composable lives) — change `remember` to `rememberSaveable`

---

### I2 — Tap-to-close when not zoomed

**What:** The fullscreen view currently requires the user to tap a back button or swipe back. A tap anywhere on the image should close fullscreen when the image is at 1× zoom (not panned/zoomed).

**How:** Add a `detectTapGestures` modifier on the fullscreen image composable that fires `fullscreenPage = null` only when the current zoom scale is ≤ 1.0. When zoomed in, taps should not close (they may pan instead).

**Files:**
- The fullscreen composable in `CrossExtensionMatchScreen.kt` — add tap gesture modifier with zoom state check
...
### I3 — Thumbnail ContentScale.Fit toggle
...
**Files:**
- The thumbnail composable in `CrossExtensionMatchScreen.kt` — add `fitMode` toggle state and icon button overlay

**Constraints:** Toggle state is per-thumbnail, local only. No preference storage needed.

---

## Phase J: Enrichment Cap — Make Configurable
...
**How:**
- Add `recommendationEnrichmentCap()` preference (Int, default 5). Reasonable range: 1–20.
...
**Files:**
- `app/src/main/java/eu/kanade/domain/source/service/SourcePreferences.kt` — add `recommendationEnrichmentCap()` preference
- `app/src/main/java/exh/recs/BrowsePersonalRecommendationsScreenModel.kt` — replace hardcoded cap with preference read
- `app/src/main/java/exh/recs/settings/RecommendationsSettingsScreen.kt` — add setting row
- `i18n-kmk/.../base/strings.xml` — `rec_enrichment_cap`, `rec_enrichment_cap_summary`

**Constraints:**
- The hardcoded value is the default — no migration of existing data needed.
...
- **Auto-suggest already-linked versions when re-searching** — future work; cross-extension match always runs fresh.
- **Per-chapter enrichment cache** — future work; enriched data not persisted to local DB.
...
docs/recommendations/KMK_RECS_POLISH_AND_REMAINING_WORK_PLAN.md  ← this file
```
...
```text
app/src/main/proto/  — current backup proto field assignments
