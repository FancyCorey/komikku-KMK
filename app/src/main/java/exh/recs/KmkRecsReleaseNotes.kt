...
object KmkRecsReleaseNotes {
    ...
    // ...") that is now expressed by its category heading instead. Rendering is covered by
    ...
    val MARKDOWN = """
        ...
        - **Source Evaluation:** the stale-reassessment button and remaining count already reflected only the actionable (workable) rows, not the total outdated count — the implementation verifies that behavior in code and added a small test-covered helper so the completion message can never be shown just because the remaining rows happen to be unreachable.
