...
fun Screen.personalRecommendationsTab(): TabContent {
    ...
    // selected for a Love/Like/Dislike action, per the behavior contract's decision not to prompt per-item during
    ...
    // KMK: `exportInputSnapshot` is captured at the
    ...
    // KMK: exact-Uri-only Remove/Keep cleanup,
...
private fun ForYouSelectionBottomBar(
    ...
    // KMK v0.8.16-fix1: width-aware layout -- all six text buttons fit a wide layout but are too
    // crowded for phone portrait. Every action stays
...
// KMK v0.8.17-fix1: shared direct-action button for the For You selection bottom bar gives every
// direct action the same icon size, spacing, label style,
...
// mirroring the `showFabMenu`-style overlay pattern already used near line 262. Until that work is
// completed, the Evaluation Mode Undo Journal (`EvaluationModeActionHistoryScreen`) records these actions and
