 ...
 *
 * The original implementation performed a direct `context as? MainActivity` cast to obtain a
...
fun ReaderScheduleDialog(
    ...
    // back press) no longer calls onSave; it now behaves like Cancel and discards the draft.
    // Only the explicit Save button commits.
            ...
            Column {
                ...
                windows.forEachIndexed { index, window ->
                    Row(
                        ...
                        // KMK v0.8.7: real Edit action opens the same AddWindowFlow
...
private fun AddWindowFlow(initial: ReaderScheduleWindow?, onDone: (ReaderScheduleWindow?) -> Unit) {
    ...
    // to prevent duplicate picker launches when recomposition occurs.
    ...
    if (stage == AddWindowStage.WEEKDAYS) {
        AlertDialog(
            ...
            confirmButton = {
                TextButton(
                    onClick = {
                        // KMK v0.8.7: whole-day windows skip both time pickers entirely.
                        stage = if (wholeDay) {
                            onDone(
    ...
    // KMK v0.8.7: 12h/24h follows the device's own preference instead of being
