 ...
 * of scope for the implementation -- see the design review for why). Rather than drive that real search pipeline (and
...
class BestVersionCompareScreenModelConfirmMigrationTest {
    ...
    fun `keepCurrentVersion reaches Done without invoking the migration use case or recording a receipt`() = runTest {
        // Direct call coverage for the "keep current" path (candidate preview, keep-current,
